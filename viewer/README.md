...
`server_py/api_server.py` is a **separate, authenticated HTTP entry point** for
an external business system to hand in a STEP file (+ optional feature-recognition
JSON) and get back a ready-to-open Viewer URL. It exists because the viewer server
itself is a loopback-bound, unauthenticated local viewer; this API is the safe
surface to expose (API key required) without touching `/__cad/*`.

Task model is asynchronous: upload returns a `jobId`, then poll for `landed`.
A landed job's `result.viewerUrl` opens the task in the viewer (port 3245); GLB
artifacts are generated lazily on first open, so `landed` means "files on disk +
URL openable". Landed files live under the shared `uploads/<jobId>/`.

Start it alongside the viewer:

```bash
VIEWER_API_KEY=change-me npm run serve:api -- --port 8420 \
  --viewer-host <this-machine-LAN-IP> --viewer-port 3245
```

Endpoints (all `/api/v1/*` require `Authorization: Bearer <api-key>`):

```text
POST /api/v1/jobs         202 {jobId, status:"received"}
    multipart/form-data:  step (file, required, .step/.stp)
                          features (text, optional, feature-recognition JSON string)
GET  /api/v1/jobs/{id}    200 {jobId, status: received|landed|failed, result?}
GET  /healthz             200 {ok: true}        (no auth)
```

```bash
# Upload STEP file + features as a text field, then poll:
curl -X POST -H "Authorization: Bearer change-me" \
  -F "step=@part.step" \
  -F 'features={"code":200,"partType":"rectangular_part"}' \
  http://<host>:8420/api/v1/jobs
curl -H "Authorization: Bearer change-me" \
  http://<host>:8420/api/v1/jobs/<jobId>
```

The `viewerUrl` advertises `--viewer-host/--viewer-port` (or `--viewer-base-url`
for a full origin); the business system must be able to reach that address too.
`VIEWER_API_KEY` / `VIEWER_UPLOAD_ROOT` are read from the environment (see
`.env.example`). Job records live in memory only; the uploaded files persist.
