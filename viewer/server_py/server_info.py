...
def normalize_dir_arg(value: str) -> str:
    """Strip a leading slash from a drive-prefixed URL path on Windows.

    ``/D:/models`` must resolve to ``D:\\models``, not ``D:\\D:\\models``.
    """
...
def _resolve_view_root(root_dir: str) -> dict:
    ...
    resolved = os.path.abspath(normalize_dir_arg(root_dir) or os.getcwd())
