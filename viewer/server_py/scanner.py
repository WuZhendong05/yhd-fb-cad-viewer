...
def is_served_cad_asset(file_path: str) -> bool:
    ...
    if extension in SOURCE_EXTENSIONS or path_is_implicit_cad_source(file_path):
        return True
    # Feature-recognition results (STEP entity id -> face groups) are served so the
    # viewer can load them at runtime via ?features=<path>. The filename must carry
    # an explicit hint to avoid treating arbitrary json/txt under the root as assets.
    if extension in (".json", ".txt") and is_feature_recognition_path(file_path):
        return True
    return False


def is_feature_recognition_path(file_path: str) -> bool:
    basename = os.path.basename(file_path).lower()
    return any(hint in basename for hint in ("feature", "识别", "特征"))
