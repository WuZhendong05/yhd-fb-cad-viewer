...
import AxesIndicator from "./viewer/AxesIndicator";
...
const VIEW_PLANE_CONTROL_SIZE = "8.25rem";
...
function readViewRotation(runtime) {
  const quaternion = runtime?.viewGroup?.quaternion;
  if (!quaternion) {
    return undefined;
  }
...
function readPerspectiveSnapshot(runtime) {
  ...
  const viewRotation = readViewRotation(runtime);
  if (viewRotation) {
    snapshot.viewRotation = viewRotation;
  }
  return snapshot;
}

function applyViewRotation(runtime, viewRotation) {
  const viewGroup = runtime?.viewGroup;
  if (!viewGroup || !Array.isArray(viewRotation) || viewRotation.length < 4) {
    return false;
  }
  const [x, y, z, w] = viewRotation.map(Number);
  if (![x, y, z, w].every(Number.isFinite)) {
    return false;
  }
  viewGroup.quaternion.set(x, y, z, w).normalize();
  return true;
}

// Resolve the model rotation for a snapshot. New snapshots carry `viewRotation`;
// legacy snapshots (camera pose only) are migrated to the equivalent rotation.
function resolveViewRotationForSnapshot(runtime, perspective) {
  if (Array.isArray(perspective.viewRotation) && perspective.viewRotation.length >= 4) {
    return perspective.viewRotation;
  }
  const derived = viewRotationForCameraPose(
    runtime?.THREE,
    perspective.position,
    perspective.target,
    perspective.up
  );
  return derived ? [derived.x, derived.y, derived.z, derived.w] : null;
}

// Keep the camera pinned: always upright (world up) and looking along the default
// direction at `target`. Only the distance (zoom) and target (pan) vary.
function pinRuntimeCamera(runtime, { target = null, distance = null } = {}) {
  if (!runtime?.THREE || !runtime?.camera || !runtime?.controls) {
    return false;
  }
  const { THREE, camera, controls } = runtime;
  const targetVec = target || controls.target;
  const resolvedDistance = Number.isFinite(distance) && distance > 0
    ? distance
    : Math.max(camera.position.distanceTo(controls.target), 1e-3);
  camera.up.set(...WORLD_UP);
  camera.position.copy(targetVec).addScaledVector(
    new THREE.Vector3(...DEFAULT_VIEW_DIRECTION).normalize(),
    resolvedDistance
  );
...
function applyPerspectiveSnapshot(runtime, perspective, { scheduleIdle = true } = {}) {
  ...
  // Camera is pinned and upright; distance comes from the snapshot's own pose so
  // legacy snapshots keep their framing.
  const legacyDistance = new runtime.THREE.Vector3(...nextPerspective.position)
    .sub(new runtime.THREE.Vector3(...nextPerspective.target)).length();
  pinRuntimeCamera(runtime, { target: runtime.controls.target, distance: legacyDistance });
  if (Number.isFinite(nextPerspective.zoom) && nextPerspective.zoom > 0) {
    runtime.camera.zoom = nextPerspective.zoom;
    runtime.camera.updateProjectionMatrix?.();
    reapplyRuntimeCameraFrameInsets(runtime);
  }
  const viewRotation = resolveViewRotationForSnapshot(runtime, nextPerspective);
  if (viewRotation) {
    applyViewRotation(runtime, viewRotation);
  } else if (runtime.viewGroup) {
    runtime.viewGroup.quaternion.identity();
  }
...
function transitionCameraToPerspectiveSnapshot(runtime, perspective, {
  ...
  const endOrthographicHalfHeight = runtime.camera?.isOrthographicCamera
    ...
    : null;
  // Model rotation: animate toward the snapshot's rotation (new snapshots carry
  // it; legacy ones are migrated). Absent both, the current rotation is kept.
  const startViewRotation = runtime.viewGroup?.quaternion?.clone() || null;
  let endViewRotation = null;
  const resolvedViewRotation = resolveViewRotationForSnapshot(runtime, nextPerspective);
  if (resolvedViewRotation) {
    endViewRotation = new runtime.THREE.Quaternion(...resolvedViewRotation).normalize();
  }
  if (
...
function stepCameraTransition(runtime, timestamp) {
  let previewOrbitMoved = false;
  // Preview auto-rotation spins the MODEL group (azimuth around the world up
  // axis) so the camera stays pinned and upright. Uses its OWN timestamp field:
  // orbitControlsLastTimestamp is owned/reset by updateOrbitControls every frame
  // (autoRotate is off), so reusing it would make this block skip forever.
  if (runtime?.previewOrbitEnabled && runtime?.viewGroup && !runtime?.cameraTransition) {
    const previous = Number(runtime.previewOrbitLastTimestamp) || 0;
    if (previous > 0) {
      const deltaSeconds = Math.min((timestamp - previous) / 1000, 0.05);
      previewOrbitMoved = applyViewRotationDelta(
        runtime,
        (2 * Math.PI / 60) * PREVIEW_AUTO_ROTATE_SPEED * deltaSeconds,
        0
      );
    }
    runtime.previewOrbitLastTimestamp = timestamp;
  }
  const transition = runtime?.cameraTransition;
  if (!transition || !runtime?.THREE || !runtime?.camera || !runtime?.controls) {
    return previewOrbitMoved;
  }
  ...
  // Model rotation: slerp the viewGroup quaternion when the transition carries
  // one (view-preset jumps / restored views). The camera itself stays pinned.
  if (
    runtime.viewGroup &&
    transition.startViewRotation &&
    transition.endViewRotation
  ) {
    runtime.viewGroup.quaternion.slerpQuaternions(
      transition.startViewRotation,
      transition.endViewRotation,
      eased
    ).normalize();
...
function transitionCameraToViewPreset(runtime, preset) {
  ...
  // Rotate the MODEL to the preset orientation; the camera stays pinned. The
  // default isometric preset maps to identity (the model's authored orientation).
  const endViewRotation = viewGroupQuaternionForPreset(runtime, preset);
  if (!endViewRotation) {
    return false;
  }
  const startViewRotation = runtime.viewGroup.quaternion.clone();
  ...
  runtime.cameraTransition = {
    ...
    easing: CAMERA_TRANSITION_EASING.EASE_IN_OUT_CUBIC,
    startPosition: runtime.camera.position.clone(),
    endPosition: runtime.camera.position.clone(),
    startTarget: currentTarget.clone(),
    endTarget: currentTarget.clone(),
    startUp: runtime.camera.up.clone(),
    endUp: runtime.camera.up.clone(),
    startViewRotation,
    endViewRotation,
    startZoom: runtime.camera.zoom,
    endZoom: runtime.camera.zoom,
    startOrthographicHalfHeight: runtime.camera?.isOrthographicCamera
      ? Number(runtime.camera.userData?.cadHalfHeight)
      : null,
    endOrthographicHalfHeight: runtime.camera?.isOrthographicCamera
      ? Number(runtime.camera.userData?.cadHalfHeight)
      : null,
    resetZoomBaselineOnComplete: false
  };
...
const CadViewer = forwardRef(function CadViewer({
  ...
  // Dragging the ViewCube rotates the camera in the same way as dragging the
  // viewport: azimuth + polar deltas from pointer movement, then re-sync the
  // cube orientation so it reflects the new view direction.
  const handleViewCubeDrag = (azimuthDelta, polarDelta) => {
    const runtime = runtimeRef.current;
    if (!runtime) {
      return;
    }
    ...
    runtime.clearRotationInertia?.();
    const rotated = applyViewRotationDelta(runtime, azimuthDelta, polarDelta);
    if (!rotated) {
      return;
    }
    activeViewPlaneFaceRef.current = "";
  ...
  // ViewCube edges and corners land on non-axis views (45° axonometric /
  // isometric), which have no VIEW_PLANE_FACES entry, so they come through as
  // raw direction + up vectors and get the same camera transition.
  const handleViewCubeNavigate = (direction, up) => {
    const runtime = runtimeRef.current;
    if (!runtime) {
      return;
    }
    ...
    const transitioned = transitionCameraToViewPreset(runtime, { direction, up });
    if (transitioned) {
      activeViewPlaneFaceRef.current = "";
      setActiveViewPlaneFace("");
      defaultPerspectiveResettingRef.current = false;
  ...
  useEffect(() => {
    ...
    runtime.previewOrbitLastTimestamp = 0;
    runtime.orbitControlsLastTimestamp = 0;
    // Preview auto-rotation spins the MODEL group (see stepCameraTransition) so
    // the camera stays pinned and upright; OrbitControls autoRotate would rotate
    // the camera instead.
    runtime.controls.autoRotate = false;
  ...
  return (
    ...
    >
      ...
      <ViewPlaneControl
        ...
        viewPlaneOffsetTop={viewPlaneOffsetTop}
        viewPlaneVerticalCenter={viewPlaneVerticalCenter}
        ...
        onViewCubeNavigate={handleViewCubeNavigate}
      />
      <AxesIndicator
        showAxes={showViewPlane && !planMode}
        previewMode={previewMode}
        isLoading={isLoading}
        hasContent={Boolean(viewportContent)}
        viewPlaneOrientation={viewPlaneOrientation}
        viewPlaneOffsetLeft={viewPlaneOffsetLeft}
        viewPlaneOffsetBottom={66}
        compact={compactViewPlane}
      />
      {error ? (
