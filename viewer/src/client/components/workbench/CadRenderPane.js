...
export default function CadRenderPane({
  ...
  // Only the screenshot CTA remains: selection no longer exposes a copy-to-clipboard
  // action, so nothing else needs a bottom overlay button.
  ...
  const ctaLabel = "复制截图";
  const ctaTitle = "复制截图到剪贴板";
  const ctaDisabled = viewerLoading || !viewportHasRenderableContent;
