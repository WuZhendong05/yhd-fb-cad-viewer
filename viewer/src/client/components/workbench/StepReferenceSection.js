...
import { ChevronLeft, ChevronRight, SquareMousePointer } from "lucide-react";
...
function DetailHeader({ typeLabel, subtitle, selector }) {
  return (
    <>
      <div className="flex items-center gap-2 px-2 pb-1.5 pt-1">
        {subtitle ? (
          <span className="min-w-0 truncate text-[15px] font-medium text-sidebar-foreground">{subtitle}</span>
        ) : null}
        <span className="shrink-0 rounded-sm bg-muted px-1 py-0.5 font-mono text-[13px] text-muted-foreground">
          {typeLabel}
        </span>
        {selector ? (
          <span className="ml-auto inline-flex shrink-0 items-center gap-1">
            <code className="max-w-[8rem] truncate rounded-sm bg-muted px-1 py-0.5 font-mono text-[13px] text-muted-foreground">
              {selector}
            </code>
          </span>
        ) : null}
...
function InfoRow({ label, children }) {
  return (
    <div className="flex items-baseline gap-3 px-2 py-1">
      <span className="w-[4.5rem] shrink-0 text-[15px] text-muted-foreground">{label}</span>
      <div className="min-w-0 flex-1 text-[15px] text-sidebar-foreground">{children}</div>
    </div>
  );
}
    ...
    <span className="flex flex-wrap items-baseline gap-x-3 gap-y-0.5 font-mono tabular-nums">
      {AXES.map((axis, index) => (
        <span key={axis.key} className="inline-flex items-baseline gap-1">
          <span className={cn("text-[11px] font-semibold", axis.className)}>{axis.key}</span>
...
function TopologyDetail({ reference }) {
  ...
  return (
    <div className="flex min-w-0 flex-col">
      <DetailHeader
        typeLabel={SELECTOR_TYPE_LABELS[type] || "引用"}
        subtitle={subtype}
      ...
      <div className="flex flex-col py-0.5">
        ...
        {radius != null ? (
          <InfoRow label="半径"><MonoValue>{`${formatNumber(radius)} mm`}</MonoValue></InfoRow>
        ) : null}
        {box ? (
          <InfoRow label="尺寸">
        ...
        {Array.isArray(center) ? (
          <InfoRow label="中心"><CoordValue vector={center} /></InfoRow>
        ) : null}
        {Array.isArray(pick.normal) ? (
          <InfoRow label="法向"><CoordValue vector={pick.normal} digits={3} /></InfoRow>
        ) : null}
        {component ? <InfoRow label="所属组件">{component}</InfoRow> : null}
      </div>
    </div>
  );
}
...
function PartDetail({ node }) {
  ...
  return (
    <div className="flex min-w-0 flex-col">
      <DetailHeader
        typeLabel={isAssembly ? "子装配体" : "组件"}
        subtitle={name}
        selector={selector}
      />
      <div className="flex flex-col py-0.5">
        {isAssembly && partCount > 0 ? (
          <InfoRow label="部件数"><MonoValue>{formatNumber(partCount, 0)}</MonoValue></InfoRow>
        ) : null}
        {box ? (
          <InfoRow label="尺寸">
            <MonoValue>{`${formatNumber(box.dims[0])} × ${formatNumber(box.dims[1])} × ${formatNumber(box.dims[2])} mm`}</MonoValue>
        ...
        {box ? <InfoRow label="中心"><CoordValue vector={box.center} /></InfoRow> : null}
        {selector ? <InfoRow label="路径">{selector}</InfoRow> : null}
      </div>
    </div>
  );
}
  ...
  if (!count) {
    return (
      <div className="flex min-h-[4.5rem] flex-col items-center justify-center gap-1.5 px-4 py-5 text-center">
        ...
        <p className="text-[15px] text-muted-foreground">Select geometry to inspect</p>
      </div>
    );
  }
    ...
    <div className="flex min-w-0 flex-col pb-2">
      {count > 1 ? (
        <div className="flex items-center justify-between gap-2 border-b border-sidebar-border/60 px-2 py-1">
          <span className="text-[15px] text-muted-foreground">{count} selected</span>
          <div className="inline-flex items-center gap-0.5">
            ...
            <span className="min-w-[2.75rem] text-center font-mono text-[15px] tabular-nums text-sidebar-foreground">
              {safeIndex + 1} / {count}
            </span>
            <Button
              type="button"
  ...
  return {
    ...
    title: (
      <span className="flex min-w-0 items-center gap-1.5">
        <span>Reference</span>
        {count > 1 ? (
          <span className="rounded-full bg-accent px-1.5 text-[13px] font-medium tabular-nums text-accent-foreground">
            {count}
          </span>
        ) : null}
      </span>
