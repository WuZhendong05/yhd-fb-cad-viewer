...
function FileEntryButton({
  ...
  const title = [
    label,
    isCodeDerivedEntry(entry) ? "Python 生成" : "",
    status.statusLabel,
    entry?.kind,
    String(entry?.file || "")
  ].filter(Boolean).join(" | ");
  ...
  return (
    <>
      <SidebarHeader className="gap-2">
        <SidebarInput
          type="search"
          placeholder="搜索文件、ID 或路径..."
          value={query}
          onChange={(event) => onQueryChange(event.target.value)}
          aria-label="搜索 CAD 文件"
          ...
          <SidebarGroup>
            <SidebarGroupContent>
              {hasMatches ? (
                ...
                <p className="px-2 py-1 text-xs text-muted-foreground">CAD 目录不可用：{catalogErrorMessage}</p>
              ) : catalogLoading ? (
                <p className="px-2 py-1 text-xs text-muted-foreground">正在加载 CAD 目录...</p>
              ) : hasEntries ? (
                <p className="px-2 py-1 text-xs text-muted-foreground">没有符合筛选条件的 CAD 条目。</p>
              ) : (
                <p className="px-2 py-1 text-xs text-muted-foreground">未找到 CAD 条目。</p>
              )}
            </SidebarGroupContent>
          </SidebarGroup>
        </ScrollArea>
