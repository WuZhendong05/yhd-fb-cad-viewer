...
export function useCadWorkspaceLayout({
  ...
  useEffect(() => {
    const handlePointerMove = (event) => {
      ...
      // The panel lives on either side: STEP/URDF/DXF/Mesh/Implicit sheets open
      // on the left (grab handle on the right edge: drag right widens), while
      // the theme sidebar opens on the right (handle on the left edge: drag
      // left widens). Pick the sign accordingly so the drag always follows the
      // mouse instead of inverting.
      const delta = event.clientX - resizeState.startX;
      const nextWidth = resizeState.side === "right"
        ? resizeState.startWidth - delta
        : resizeState.startWidth + delta;
      // Drag to the minimum width and stop there — never auto-close the panel
      // mid-drag. The sheet can still be closed via its toggle button, which
      // is what users expect from a resize handle.
