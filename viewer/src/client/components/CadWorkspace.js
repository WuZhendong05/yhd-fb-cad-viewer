...
import { buildFeatureTree, resolveFeatureFaceIds } from "@/workbench/featureRecognition";
import {
...
export default function CadWorkspace({
  ...
  // Single-file preview mode: the URL names an exact ?file=, so the backend returns
  // just that entry (no directory scan) and the file-tree sidebar stays hidden.
  const singleFileMode = Boolean(explicitFileParam);
  ...
  const [sidebarOpen, setSidebarOpen] = useState(() => (
    singleFileMode ? false : readDirectorySessionState().fileViewerOpen
  ));
  ...
  const effectiveSidebarOpen = sidebarOpen && !previewMode && !singleFileMode;
  const desktopSidebarOpen = isDesktop && effectiveSidebarOpen && !previewMode && !singleFileMode;
  ...
  }, [isDesktop, setTabToolsOpen, singleFileMode]);
  ...
  // Feature recognition (方案2): URL ?features=<path> 指向外部识别结果 JSON；
  // relatedEntityIds 是 STEP 实体 id，通过 GLB 拓扑的 stepEntityId 列关联到组件面。
  const featuresFileParam = readFeaturesParam();
  const [featureRecognitionPayload, setFeatureRecognitionPayload] = useState(null);
  const [featureRecognitionError, setFeatureRecognitionError] = useState("");
  useEffect(() => {
    if (!selectedEntry || !featuresFileParam) {
      setFeatureRecognitionPayload(null);
      setFeatureRecognitionError("");
      return;
    }
    let cancelled = false;
    ...
    fetch(url)
      ...
      .then((payload) => {
        if (!cancelled) {
          setFeatureRecognitionPayload(payload);
          setFeatureRecognitionError("");
        }
      })
      .catch((err) => {
        if (!cancelled) {
          setFeatureRecognitionPayload(null);
          setFeatureRecognitionError(err instanceof Error ? err.message : String(err));
        }
      });
    return () => {
      cancelled = true;
    };
  }, [featuresFileParam, selectedEntry]);
  const recognitionFeatures = useMemo(() => {
    if (!featureRecognitionPayload) {
      return [];
    }
    try {
      return buildFeatureTree(featureRecognitionPayload);
    } catch (err) {
      // partType 不在支持范围内：buildFeatureTree 抛出提示，这里转为提示节点展示，
      // 避免渲染崩溃。
      ...
      return [{
        ...
        children: [{
          ...
          label: "支持的分类：方件 (rectangular_part)、方圆件 (round_rectangular_part)、圆件 (round_pure_part)",
          leaf: true
        }]
      }];
    }
  }, [featureRecognitionPayload]);
  const resolvedRecognitionFeatures = useMemo(
    () => resolveFeatureFaceIds(recognitionFeatures, selectedSelectorRuntime),
    [recognitionFeatures, selectedSelectorRuntime]
  );
  ...
  // Extra lookup aliases for a reference, so the map answers by any of its
  // identifiers (id, topology ref, selector, display selector) — selection and
  // highlight resolve references by whichever identifier the source produced.
  const addReferenceLookupKeys = (map, reference) => {
    ...
    for (const alias of [reference.topologyReferenceId, reference.displaySelector, reference.normalizedSelector]) {
      const normalizedAlias = String(alias || "").trim();
      if (normalizedAlias && normalizedAlias !== referenceId) {
        map.set(normalizedAlias, reference);
      }
    }
  };
  ...
  const filenameLoadActivity = useMemo(() => {
    ...
    if ([RENDER_FORMAT.STEP, RENDER_FORMAT.STL, RENDER_FORMAT.THREE_MF, RENDER_FORMAT.GLB].includes(effectiveRenderFormat) && meshViewerLoading) {
      ...
      return {
        loading: true,
        label: selectedEntryHasMesh ? (activeMeshLoadStage || "正在加载网格") : "正在构建",
        title: viewerLoadingLabel
      };
    }
    ...
    if (effectiveRenderFormat === RENDER_FORMAT.STEP && assemblyHydrationLoading) {
      ...
      return {
        loading: true,
        label: activeMeshLoadStage || "正在加载网格",
        title: "正在加载装配体网格"
      };
    }
    ...
    if (effectiveRenderFormat === RENDER_FORMAT.STEP && referenceSelectionStatus === REFERENCE_STATUS.LOADING) {
      return {
        loading: true,
        label: referenceLoadStage || "正在加载拓扑",
        title: "正在加载可选择的拓扑结构"
      };
    }
  ...
  const viewportFrameInsets = {
    ...
    right: themeEditing ? activeSheetWidth : 0,
    bottom: 0,
    // STEP 模型面板（左侧展开）时视口整体右移避让，与右侧主题面板的左移避让对称；
    // max 覆盖面板比文件列表窄的边界情况。
    left: fileSheetOpen ? Math.max(activeSidebarWidth, activeSheetWidth) : activeSidebarWidth
  };
  ...
  const drawingToolOptions = [
    { id: DRAWING_TOOL.FREEHAND, label: "自由绘制", Icon: PenTool },
    { id: DRAWING_TOOL.LINE, label: "直线", Icon: Minus },
    { id: DRAWING_TOOL.ARROW, label: "箭头", Icon: ArrowRight },
    { id: DRAWING_TOOL.DOUBLE_ARROW, label: "双向箭头", Icon: ArrowLeftRight },
    { id: DRAWING_TOOL.RECTANGLE, label: "矩形", Icon: Square },
    { id: DRAWING_TOOL.CIRCLE, label: "圆形", Icon: Circle },
    { id: DRAWING_TOOL.FILL, label: "填充", Icon: PaintBucket },
    { id: DRAWING_TOOL.ERASE, label: "擦除", Icon: Eraser }
  ];
    ...
    >
      <div className="fixed inset-0 z-0">
        <CadRenderPane
          ...
          viewPlaneOffsetRight={viewportFrameInsets.right + 80}
          viewPlaneOffsetLeft={fileSheetOpen ? activeSheetWidth + 66 : 66}
          ...
          <div className="flex h-full min-w-0">
            ...
            {selectedFileSheetKind === "step" ? (
              <StepFileSheet
                ...
                recognitionFeatures={resolvedRecognitionFeatures}
                recognitionError={featureRecognitionError}
                onSelectFeatureFaces={setSelectedReferenceIds}
            ...
            <div className="pointer-events-none absolute inset-0 min-w-0 overflow-hidden">
              <FloatingToolBar
                previewMode={previewMode}
                selectedEntry={selectedEntry}
                renderFormat={effectiveRenderFormat}
