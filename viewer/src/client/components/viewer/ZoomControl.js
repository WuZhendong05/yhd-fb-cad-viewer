...
const DISPLAY_TOOLBAR_BUTTON_CLASSES = "grid size-8 shrink-0 place-items-center rounded-sm text-sidebar-foreground/70 transition hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/45 disabled:pointer-events-none disabled:opacity-50";
  ...
  return (
    ...
    >
      ...
      <input
        ...
        inputMode="numeric"
        aria-label="缩放百分比"
        className="h-8 w-11 min-w-0 rounded-sm border border-transparent bg-transparent px-0 text-center text-lg font-medium tabular-nums text-sidebar-foreground outline-none transition focus-visible:border-ring focus-visible:bg-sidebar-accent/40 focus-visible:ring-2 focus-visible:ring-ring/35"
        value={inputValue}
        onFocus={(event) => {
      ...
      >
        <RotateCcw className="size-4" strokeWidth={2.1} aria-hidden="true" />
      </button>
    </div>
  );
}
