...
export function useImplicitRaymarch({
  ...
  useEffect(() => {
    ...
    // Feed the SHARED camera to the shader every frame. The model rotates inside
    // viewGroup (the camera is pinned), so the SDF — which is evaluated in world
    // space — must be sampled as if the rays were rotated by the inverse model
    // rotation. A lightweight virtual camera whose pose is pre-rotated by that
    // inverse does exactly this without touching the real camera.
    ...
    quad.onBeforeRender = (renderer, _scene, camera) => {
      ...
      if (viewGroup?.quaternion && viewGroup.quaternion.lengthSq() > 1e-8) {
        const inverseView = viewGroup.quaternion.clone().invert();
        virtualCamera.isOrthographicCamera = camera.isOrthographicCamera;
        virtualCamera.position.copy(camera.position).applyQuaternion(inverseView);
        virtualCamera.quaternion.copy(camera.quaternion).premultiply(inverseView);
        virtualCamera.projectionMatrix.copy(camera.projectionMatrix);
        virtualCamera.projectionMatrixInverse.copy(camera.projectionMatrixInverse);
        virtualCamera.matrixWorld.compose(virtualCamera.position, virtualCamera.quaternion, oneScale);
        virtualCamera.matrixWorldInverse.copy(camera.matrixWorldInverse);
        renderCamera = virtualCamera;
      }
      updateImplicitCadMaterialUniforms(material, renderCamera, drawingBufferSize.x, drawingBufferSize.y);
