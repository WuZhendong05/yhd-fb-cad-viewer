...
test("default step arrangement puts the trees on top and everything else on the bottom", () => {
  const arrangement = defaultFileSheetTabArrangement(
    "step",
    ["features", "tree", "parameters", "display", "theme", "metadata"]
  );
  assert.equal(arrangement.split, true);
  assert.deepEqual(arrangement.top, ["features", "tree"]);
  assert.deepEqual(arrangement.bottom, ["parameters", "display", "theme", "metadata"]);
  assert.equal(arrangement.ratio, 2 / 3);
});
...
test("split ratio is clamped", () => {
  ...
  assert.equal(clampSplitRatio("nope"), 2 / 3);
