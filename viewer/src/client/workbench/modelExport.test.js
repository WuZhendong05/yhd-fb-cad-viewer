...
test("exportItemLabel: a native format is a download, everything else is an export", () => {
  assert.equal(exportItemLabel("step"), "下载 STEP");
  assert.equal(exportItemLabel("dxf"), "下载 DXF");
  assert.equal(exportItemLabel("3mf"), "导出 3MF");
  assert.equal(exportItemLabel("stl"), "导出 STL");
  assert.equal(exportItemLabel("glb"), "导出 GLB");
});
